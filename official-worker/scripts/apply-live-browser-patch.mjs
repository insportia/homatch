...
patch('src/orchestrator/ResearchOrchestrator.ts', [
  ...
  [
    ...
    "browser = browser || (await connectResearchBrowser());",
    'remote browser launch'
  ],
  [
    "const ctx = await browser.newContext({ locale: 'ka-GE', acceptDownloads: true, viewport: { width: 1440, height: 1000 } });\n    const page = await ctx.newPage();",
    "const ctx = browser.contexts()[0] || (await browser.newContext({ locale: 'ka-GE', acceptDownloads: true, viewport: { width: 1440, height: 1000 } }));\n    const stalePages = ctx.pages();\n    for (const oldPage of stalePages) await oldPage.close().catch(() => {});\n    const page = await ctx.newPage();\n    await page.setViewportSize({ width: 1440, height: 1000 }).catch(() => {});",
    'Browserless default context'
  ],
  [
    "await ctx.close().catch(() => {});\n      return { result, keep: false };",
    "for (const p of ctx.pages()) await p.close().catch(() => {});\n      return { result, keep: false };",
    'keep remote default context alive between source steps'
  ],
  [
    "await ctx.close().catch(() => {});\n      return {\n        result:",
    "for (const p of ctx.pages()) await p.close().catch(() => {});\n      return {\n        result:",
    'keep remote context on workflow exception'
  ],
  [
    "await session.ctx.close().catch(() => {});\n    this.sessions.delete(jobId);",
    "for (const p of session.ctx.pages()) await p.close().catch(() => {});\n    this.sessions.delete(jobId);",
    'resume cleanup without killing Browserless context'
  ],
  [
    "await session.ctx.close().catch(() => {});\n    const browser = session.browser;",
    "for (const p of session.ctx.pages()) await p.close().catch(() => {});\n    const browser = session.browser;",
    'skip cleanup without killing Browserless context'
  ]
]);
