...
// domIframes (2026-09-04, round 2): mygov's naprweb.reestri.gov.ge search
// iframe was confirmed present via a plain curl fetch of the service page's
// raw HTML earlier this session, yet job 4fddea39-...'s frameUrls (built
// from Playwright's own p.frames()) never listed it — only the top-level
// page and the reCAPTCHA anchor iframe. That means either the <iframe> tag
// genuinely isn't in the live-rendered DOM at the time we look (added later
// by some interaction, or gated on something curl doesn't hit), or it *is*
// in the DOM but Playwright isn't attaching it as a frame for some reason
// (sandboxed/blocked). This reads the DOM directly so the next live run can
// tell those two cases apart instead of guessing.
...
app.get('/health',(_q,r)=>r.json({ok:true,service:'homatch-official-worker',version:'1.2.1',playwright:true,pdfExtraction:true,sourceAdapters:['tas-frame-aware','msmap-spa','mygov-service'],humanSessionControls:true,evidenceValidation:true,evidenceModel:'v2-status-enum-2026-09-04'}));
...
app.listen(PORT,'0.0.0.0',()=>console.log(`homatch-official-worker 1.2.1 listening on ${PORT}`));
