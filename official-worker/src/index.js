...
async function collect(p,key,sr){const src=SOURCES[key],ls=await pageLinks(p),cap=await challenge(p),docs=cap?[]:await pdfEvidence(p,ls),pageText=await text(p),q=(sr?.query||'').replace(/\s/g,''),pt=pageText.replace(/\s/g,'');const foundControl=!!sr?.found,submitted=!!sr?.submitted,noResult=hasNoResultPhrase(pageText),echoesQuery=submitted&&(pt.includes(q)||docs.some(d=>d.text?.replace(/\s/g,'').includes(q)));let status,resultConfirmed=false,noResultConfirmed=false,authRequired=false;if(cap){status='WAITING_HUMAN'}else if(!foundControl){authRequired=looksAuthGated(pageText);status=authRequired?'AUTH_REQUIRED':'SEARCH_CONTROL_NOT_FOUND'}else if(!submitted){status='SUBMIT_FAILED'}else if(noResult){status='NO_RESULT_CONFIRMED';noResultConfirmed=true}else if(echoesQuery){status='SEARCH_CONFIRMED';resultConfirmed=true}else{status='SUBMITTED_UNCONFIRMED'}
return{source:key,sourceName:src.name,sourceClass:src.class,sourceUrl:src.url,finalUrl:p.url(),adapter:sr?.adapter||null,frameUrl:sr?.frameUrl||null,retrievalMethod:submitted?'OFFICIAL_FORM_RESULT':'NO_VERIFIED_SEARCH',searchControlFound:foundControl,queryEntered:foundControl,submitted,submissionConfirmed:submitted&&!noResult&&!echoesQuery?false:submitted,resultConfirmed,noResultConfirmed,authRequired,searched:submitted,resultValidated:resultConfirmed,status,captcha:!!cap,retrievedAt:now(),pageText,links:ls,documents:docs,documentsDiscovered:ls.filter(isRealDocumentLink).length,documentsExtracted:docs.filter(d=>d.parsed).length,documentLinks:ls.filter(isRealDocumentLink),error:status==='SEARCH_CONTROL_NOT_FOUND'?'source adapter could not locate a matching search control':status==='SUBMIT_FAILED'?'source adapter located the control but could not submit the search':null}}
