...
async function mygovAdapter(p,q,ctx){const hints=['input[placeholder*="საკადასტრო" i]','input[name*="cad" i]','input[id*="cad" i]'];let hit=await interact(p,q,hints);let activePage=null;if(!hit.found){const poll=await pollForIframe(p,/reestri\.gov\.ge|naprweb/i,{timeoutMs:15000,pollMs:1000});hit.trace.push({action:'DOM_IFRAME_SCAN',srcs:poll.srcs,waitedMs:poll.waitedMs,found:poll.found});const target=poll.found?poll.src:null;if(target){let full=target;try{full=new URL(target,p.url()).toString()}catch{}const p2=await ctx.newPage();try{await p2.goto(full,{waitUntil:'domcontentloaded',timeout:30000});await p2.waitForTimeout(2000);hit.trace.push({action:'DIRECT_NAVIGATE',url:full});const hit2=await interact(p2,q,hints);hit.trace.push(...hit2.trace);if(hit2.found){hit=hit2;activePage=p2}else{await p2.close().catch(()=>{})}}catch(e){hit.trace.push({action:'DIRECT_NAVIGATE_FAILED',error:String(e)});await p2.close().catch(()=>{})}}}if(!hit.found)return{adapter:'MYGOV_SERVICE',found:false,trace:hit.trace,activePage};const sig=await waitForResultSignal(hit.frame,hit.before,q);hit.trace.push({action:'RESULT',changed:sig.changed,signal:sig.signal,queryEchoed:sig.queryEchoed,waitedMs:sig.waitedMs});return{adapter:'MYGOV_SERVICE',found:true,submitted:hit.sub.ok,submitAction:hit.sub.method,frameUrl:hit.frame.url(),searchControlUsed:hit.selector,fillVerified:true,resultChanged:sig.changed,resultTextAfter:sig.after,trace:hit.trace,activePage}}
...
app.get('/health',(_q,r)=>r.json({ok:true,service:'homatch-official-worker',version:'1.3.1',playwright:true,pdfExtraction:true,sourceAdapters:['tas-frame-aware','msmap-spa','mygov-service+dom-iframe-fallback','generic'],humanSessionControls:true,evidenceValidation:true,evidenceModel:'v3.1-network-proof-plus-ranked-candidates-2026-09-04'}));
...
app.listen(PORT,'0.0.0.0',()=>console.log(`homatch-official-worker 1.3.1 listening on ${PORT}`));
