...
async function msmapAdapter(p,q){
  ...
  hit.trace.push({action:'RESULT',usedFallbackCandidate,networkConfirmed:netConfirmed,suggestionFound:sug.found,suggestionPrefix:sug.prefix||null,suggestionClicked:clicked,mapRedrawConfirmed:mapConfirmed,mapRedrawRequestCount:netFeatureCount});
  const forceStatus=sug.found?'SEARCH_CONFIRMED':(netConfirmed?'NO_RESULT_CONFIRMED':undefined);
  const forceReason=sug.found?`MSMAP's unified-search suggested a match at cadastral prefix ${sug.prefix}${mapConfirmed?`, and selecting it triggered ${netFeatureCount} map-redraw request(s) (geoserver/tileserver) confirming the parcel was located and its boundary drawn`:' (selecting it did not trigger a detectable map redraw, but the suggestion itself is the network-generated search result)'}`:(netConfirmed?'MSMAP\'s unified-search ran (network-confirmed POST to core-api/v1/search/unified-search) and returned no matching suggestion at any cadastral-segment prefix of the query':undefined);
  return{adapter:'MSMAP_SPA_SUGGESTION',found:true,submitted:!!hit.sub?.ok,submitAction:hit.sub?.method||null,frameUrl:hit.frame?.url?hit.frame.url():null,searchControlUsed:hit.selector,fillVerified:true,resultChanged:sug.found||netConfirmed,networkConfirmed:netConfirmed,resultTextAfter:forceReason||null,forceStatus,forceReason,trace:hit.trace,candidateInputs:usedFallbackCandidate?hit.candidates:null}
}
...
app.get('/health',(_q,r)=>r.json({ok:true,service:'homatch-official-worker',version:'1.5.0',playwright:true,pdfExtraction:true,documentIntegrity:'sha256+title+date',historicalComparison:true,sourceAdapters:['tas-frame-aware','msmap-spa-suggestion','mygov-service+dom-iframe-fallback','generic'],statuses:['SEARCH_CONFIRMED','NO_RESULT_CONFIRMED','SUBMITTED_UNCONFIRMED','SUBMIT_FAILED','AUTH_REQUIRED','SEARCH_CONTROL_NOT_FOUND','BLOCKED','WAITING_HUMAN','FAILED'],humanSessionControls:true,evidenceValidation:true,evidenceModel:'v3.4-msmap-suggestion-confirmed-2026-09-04'}));
