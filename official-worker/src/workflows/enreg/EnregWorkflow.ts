...
const SOURCE_META = { name: 'Official Government Sources', class: 'OFFICIAL_GOVERNMENT', url: SOURCE_URL };
...
async function findService179Frame(page: Page): Promise<Frame> {
  for (let i = 0; i < 120; i++) {
    for (const frame of page.frames()) {
      ...
      if ((await input.count().catch(() => 0)) && (await input.isVisible().catch(() => false))) return frame;
    }
    await sleep(500);
  }
...
async function enregCaptchaVisible(frame: Frame): Promise<boolean> {
  for (const selector of ['#capture_gate', 'img[src*="icaptcha.php" i]', 'img[src*="captcha" i]', 'input[name*="captcha" i]', 'input[id*="captcha" i]']) {
...
async function readHttp(page: Page, url: string) {
  ...
  if (contentType.includes('pdf') || buffer.subarray(0, 4).toString() === '%PDF') return await readPdf(buffer, url);
  const rawText = buffer.toString('utf8');
  return { url, label: 'Official document', documentType: 'ONLINE_DOCUMENT', sourceCategory: 'OFFICIAL_DOCUMENT', evidenceLevel: 'OFFICIAL', retrievalMethod: 'DOCUMENT_RETRIEVED_AND_PARSED', rawText, textExtracted: !!rawText.trim(), complete: !!rawText.trim() };
}

async function readAfterSignedClick(context: BrowserContext, hostPage: Page, beforePages: Set<Page>, downloadPromise: Promise<any>) {
...
export async function runEnregWorkflow(
  ...
  try {
    ...
    const continueFromEntityPage = async () => {
      const companyDetailText = await frame.locator('body').innerText().catch(() => '');
      if (entities && companyDetailText) entities.scanText(companyDetailText, { source: 'enreg', sourceDocument: SOURCE_URL, retrievedAt: new Date().toISOString() });
      const blob = frame.locator('#tabs-1 > table > tbody > tr:nth-child(1) > td:nth-child(1) > a > img[src*="blob.png"]').first();
      await blob.waitFor({ state: 'visible', timeout: 20000 });
      ...
      if (entities && applicationDetailText) entities.scanText(applicationDetailText, { source: 'enreg', sourceDocument: SOURCE_URL, retrievedAt: new Date().toISOString() });
      ...
      if (entities && doc.rawText) entities.scanText(doc.rawText, { source: 'enreg', sourceDocument: doc.url, retrievedAt: new Date().toISOString() });
      return buildResult('ENREG_EXHAUSTED', idCode, true, true, [doc], trace, null, true, { companyDetailText, applicationDetailText });
    };

    if (opts.skipGoto) {
      trace.record({ stateBefore: 'WAITING_HUMAN', action: 'RESUME_SAME_SESSION', actualOutcome: 'HUMAN_VERIFICATION_COMPLETED', stateAfter: 'ENTITY_PAGE_READ' });
      return await continueFromEntityPage();
    }
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (clean(await row.innerText().catch(() => '')).includes(idCode)) { companyRow = row; break; }
    }
    if (!companyRow) {
    ...
    if (await enregCaptchaVisible(frame)) {
      trace.record({ stateBefore: 'CORRECT_ENTITY_MATCHED', action: 'DETECT_HUMAN_VERIFICATION', actualOutcome: 'CAPTCHA_VISIBLE', stateAfter: 'WAITING_HUMAN' });
      return buildResult('WAITING_HUMAN', idCode, true, true, [], trace, null, false);
    }
    return await continueFromEntityPage();
  } catch (e) {
  ...
  function buildResult(state: string, value: string | null, matched: boolean, infoIconClicked: boolean, documents: any[], tr: BrowserTrace, error: string | null, fullChain: boolean, context?: { companyDetailText?: string; applicationDetailText?: string }): LegacySourceResult {
    const traversal = computeEnregTraversal({ searchMethod: value ? 'ID_CODE' : null, searchValue: value, exactEntityMatched: matched, infoIconClicked, entityPageOpened: matched && infoIconClicked, latestApplicationDate: null, latestApplicationOpened: fullChain, preparedDocumentsOpened: fullChain, latestRegistryExtractOpened: fullChain, fullExtractRead: fullChain, historicalRelevantRecordsRead: fullChain, searchControlNotFound: false, submitFailed: false, captcha: state === 'WAITING_HUMAN', failed: state === 'FAILED', noResultConfirmed: state === 'NO_RESULT_CONFIRMED' });
    const legacyStatus = state === 'ENREG_EXHAUSTED' ? 'SEARCH_CONFIRMED' : state;
    const workflowResult: WorkflowResult = { source: 'enreg', state, completed: state === 'ENREG_EXHAUSTED', skipped: false, discoveredItems: matched ? 1 : 0, visitedItems: infoIconClicked ? 1 : 0, discoveredDocuments: documents.length, readDocuments: documents.filter((d: any) => d.complete).length, unvisitedRelevantItems: matched && state !== 'ENREG_EXHAUSTED' ? 1 : 0, evidenceIds: [], trace: tr.all };
    return { source: 'enreg', sourceName: SOURCE_META.name, sourceClass: SOURCE_META.class, sourceUrl: SOURCE_META.url, startUrl: SOURCE_META.url, finalUrl: SOURCE_META.url, frameUrls: page.frames().map((f) => f.url()), searchControlUsed: value ? '#s_legal_person_idnumber' : null, queryEntered: value, submitAction: value ? 'CLICK #s_search_persons_form button.submit_button' : null, resultContext: error || context?.applicationDetailText || context?.companyDetailText || `Official government source reached ${state}`, resultConfirmed: legacyStatus === 'SEARCH_CONFIRMED', noResultConfirmed: legacyStatus === 'NO_RESULT_CONFIRMED', resultValidated: legacyStatus === 'SEARCH_CONFIRMED', status: legacyStatus, traversal, retrievedAt: new Date().toISOString(), documents, discoveredEntities: [], forEntity, error, workflowResult };
  }
}
