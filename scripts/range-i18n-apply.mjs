...
function main() {
  ...
  for (const [key, values] of entries) {
    if (seen.has(key)) {
      console.error(`[range-i18n] FATAL: ${key} is defined twice across the data files`);
      fatal += 1;
    }
    seen.add(key);

    if (!Array.isArray(values) || values.length !== LANGS.length) {
      console.error(`[range-i18n] FATAL: ${key} has ${values?.length} values, expected ${LANGS.length}`);
      fatal += 1;
      continue;
    }
    ...
    for (let i = 0; i < LANGS.length; i += 1) {
      ...
      if (typeof value !== 'string' || value.trim().length === 0) {
        console.error(`[range-i18n] FATAL: ${key} has an empty ${LANGS[i]} value`);
        fatal += 1;
        continue;
      }
      if (holes(value) !== expected) {
        console.error(
          `[range-i18n] FATAL: ${key} (${LANGS[i]}) has placeholders [${holes(value)}], English has [${expected}]`,
        );
        fatal += 1;
      }
      ...
      if ((LANGS[i] === 'ka' || LANGS[i] === 'ar' || LANGS[i] === 'he') && value === values[0]) {
        if (!/^[\p{Lu}\p{Ll}\s.,'’—–-]*$/u.test(value) || value.length > 24) {
          console.error(`[range-i18n] FATAL: ${key} (${LANGS[i]}) is identical to the English`);
          fatal += 1;
        }
      }
    }
  ...
  if (fatal > 0) {
    console.error(`[range-i18n] ${fatal} problem(s); nothing was written.`);
    process.exit(1);
  }
  ...
  for (let langIndex = 0; langIndex < LANGS.length; langIndex += 1) {
    ...
    if (start === -1) {
      console.error(`[range-i18n] FATAL: could not find the ${lang} bundle opener`);
      process.exit(1);
    }
    const end = source.indexOf('\n};', start);
    if (end === -1) {
      console.error(`[range-i18n] FATAL: could not find the end of the ${lang} bundle`);
      process.exit(1);
    }
  ...
  if (collisions.size > 0 && !overwrite) {
    console.error(
      `[range-i18n] ${collisions.size} key(s) already exist with a different value and were LEFT ALONE:\n  ` +
        [...collisions].join('\n  ') +
  ...
  console.log(
    `[range-i18n] ${added} added, ${replaced} replaced, ${unchanged} already current — ` +
