...
function Floor({ index, progress, reduced }: { index: number; progress: any; reduced: boolean | null }) {
  ...
  return (
    <motion.div
      ...
      style={{ top: `${64 + index * 29}px`, y: reduced ? 0 : y, x: reduced ? 0 : x, rotateZ: reduced ? 0 : rotateZ, opacity }}
    >
...
export const HomepageBuildingSequence: React.FC = () => {
  ...
  useEffect(() => {
    if (location.pathname !== '/') { setMount(null); return; }
    let host: HTMLElement | null = null;
    let cancelled = false;
    let attempts = 0;
    const place = () => {
      if (cancelled) return;
      const hero = document.querySelector('.min-h-screen.bg-background.text-foreground.overflow-x-hidden > header + section:first-of-type');
      if (hero instanceof HTMLElement) {
        host = document.createElement('div');
        host.setAttribute('data-homatch-building-sequence', 'true');
        hero.insertAdjacentElement('afterend', host);
        setMount(host);
        return;
      }
      if (++attempts < 20) window.setTimeout(place, 50);
    };
    place();
    return () => {
      cancelled = true;
  ...
  return createPortal(
    <section ref={sectionRef} aria-label="Homatch property intelligence visualization" className="relative h-[185vh] md:h-[220vh] border-y border-border bg-background">
      <div className="sticky top-14 h-[calc(100vh-3.5rem)] min-h-[560px] overflow-hidden flex items-center justify-center">
        ...
        <motion.div className="relative w-[330px] h-[500px] sm:w-[430px] md:w-[560px] md:h-[560px] [perspective:1100px]" style={{ scale: reduced ? 1 : buildingScale, y: reduced ? 0 : buildingY, rotateX: reduced ? 0 : buildingRotateX }}>
          ...
          {Array.from({ length: FLOOR_COUNT }).map((_, i) => <Floor key={i} index={i} progress={progress} reduced={reduced} />)}
          <motion.div className="absolute left-1/2 -translate-x-1/2 w-[300px] sm:w-[390px] md:w-[470px] h-px bg-gradient-to-r from-transparent via-primary to-transparent shadow-[0_0_16px_hsl(var(--primary)/0.75)] z-20" style={{ y: reduced ? 250 : scanY, opacity: reduced ? 0 : scanOpacity }} />
          <motion.div style={{ opacity: chipOpacity, scale: chipScale }} className="absolute left-[-8px] md:left-[-70px] top-[150px] flex items-center gap-2 rounded-xl border border-primary/25 bg-card/90 backdrop-blur-md px-3 py-2 shadow-xl"><ScanSearch className="h-4 w-4 text-primary" /><span className="h-1.5 w-12 rounded-full bg-primary/35" /></motion.div>
          <motion.div style={{ opacity: chipOpacity, scale: chipScale }} className="absolute right-[-8px] md:right-[-76px] top-[235px] flex items-center gap-2 rounded-xl border border-primary/25 bg-card/90 backdrop-blur-md px-3 py-2 shadow-xl"><FileCheck2 className="h-4 w-4 text-primary" /><span className="h-1.5 w-14 rounded-full bg-primary/35" /></motion.div>
          <motion.div style={{ opacity: chipOpacity, scale: chipScale }} className="absolute left-[8px] md:left-[-48px] bottom-[74px] flex items-center gap-2 rounded-xl border border-primary/25 bg-card/90 backdrop-blur-md px-3 py-2 shadow-xl"><ShieldCheck className="h-4 w-4 text-primary" /><span className="h-1.5 w-10 rounded-full bg-primary/35" /></motion.div>
        </motion.div>

        <div className="absolute top-7 md:top-10 left-1/2 -translate-x-1/2 flex items-center gap-2 text-primary/80 pointer-events-none"><Building2 className="h-4 w-4" /><span className="text-[10px] md:text-xs font-semibold tracking-[0.24em] uppercase whitespace-nowrap">Homatch Intelligence</span></div>
