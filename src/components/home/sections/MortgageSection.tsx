            ...
            {POINTS.map(point => (
              <li key={point.key} className="flex gap-3.5">
                ...
                <span className="min-w-0">
                  <span className="block text-[15px] font-semibold leading-snug text-foreground">{t(point.title)}</span>
                  <span className="mt-1 block text-pretty text-[13.5px] leading-relaxed text-ink-soft sm:text-sm">
                    {t(point.desc)}
                  </span>
                </span>
              </li>
        ...
        <div className="min-w-0 overflow-hidden rounded-[1.1rem] border border-foreground/15 bg-card shadow-hover" role="img" aria-label={t('mp_mortgage_title')}>
          <div className="flex items-center justify-between gap-3 border-b border-foreground/[0.12] px-5 py-4 sm:px-6">
            ...
            <div className="flex shrink-0 items-center gap-2" aria-hidden="true">
              <span className="grid h-6 w-6 place-items-center rounded-[0.35rem] bg-primary text-[11px] font-semibold text-primary-foreground">A</span>
              <span className="grid h-6 w-6 place-items-center rounded-[0.35rem] border border-foreground/20 text-[11px] font-semibold text-muted-foreground">B</span>
            </div>
          </div>
          ...
          <dl className="divide-y divide-foreground/10">
            {ROWS.map(row => (
              <div key={row} className="flex items-center gap-3 px-5 py-3.5 sm:px-6">
                <dt className="min-w-0 flex-1 text-[13px] text-ink-soft sm:text-sm">{t(row)}</dt>
          ...
          <div className="flex items-center justify-between gap-4 bg-primary px-5 py-5 text-primary-foreground sm:px-6">
            <p className="min-w-0 text-[13px] font-semibold uppercase tracking-[0.14em] text-gold sm:text-sm">
