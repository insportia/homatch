// HOMATCH INVESTMENT INTELLIGENCE — the things you click.
//
...
// the investor to produce a figure out of nothing, while three plausible
// amounts ask them to recognise one. Recognition is faster, and on a phone
// it is the difference between finishing the analysis and abandoning it.
//
// WHERE A NUMBER CAME FROM IS PART OF THE CONTROL
//
// A suggestion computed from what the investor already entered, a market
// observation, a trade convention and a round illustration are four
// different kinds of claim, and a chip that renders them identically is
// quietly dishonest. Every option carries its source and shows it.
//
// TAP TARGETS
//
...
import type { Preset, PresetKind } from '@/investment/strategies/presets';
import type { FieldOption, ValueKind } from '@/investment/strategies/definitions';
import { formatMoney, formatNumber, formatPercent, intlLocaleFor } from './primitives';

/* ── Where a value came from ────────────────────────────────────────── */

export type ValueSource = PresetKind | 'USER';

/**
 * The four-way label the product rule demands.
 *
 * Market evidence and an illustration must never look the same, so they do
 * not share a colour, a weight or a word.
 */
