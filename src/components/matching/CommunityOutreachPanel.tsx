            ...
            {items.map((item) => {
              ...
              return (
                <div key={c.id} className="rounded-xl border border-border overflow-hidden">
                  ...
                  >
                    <Badge variant="outline" className={`text-[13px] shrink-0 ${PLATFORM_COLORS[c.platform] ?? ''}`}>{c.platform}</Badge>
                    <div className="flex-1 min-w-0">
                    ...
                    {isPosted && <Badge className="text-[13px] bg-green-500/15 text-green-400 border-green-500/30 shrink-0">{t('community_status_posted')}</Badge>}
                    {isOpen ? <ChevronUp className="h-4 w-4 text-muted-foreground shrink-0" /> : <ChevronDown className="h-4 w-4 text-muted-foreground shrink-0" />}
                  </div>
                  {isOpen && (
                    <div className="p-3 pt-0 space-y-3 border-t border-border">
                      {c.posting_allowed === false && (
                        <p className="text-[14px] text-muted-foreground italic pt-3">{t('community_no_direct_post')}</p>
                      )}
                      {c.housing_focus === 'secondary' && (
                        <p className="text-[14px] text-muted-foreground italic pt-3">{t('community_secondary_note')}</p>
                      )}
