    ...
    return (
      <Popover>
        <PopoverTrigger asChild>
          <button className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[14px] font-medium cursor-pointer hover:opacity-80 transition-opacity ${badgeClass}`}>
      ...
      <PopoverTrigger asChild>
        <div className="flex items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity">
          <div className="flex items-center gap-1.5 p-2 bg-secondary rounded-lg border border-border">
            ...
            <div>
              <p className="text-[13px] text-muted-foreground">{t('trust_score')}</p>
...
function TrustPopoverContent({ score, risks, scoreColor, t }: {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-[13px] text-muted-foreground border-t border-border pt-2 leading-relaxed">
        {t('trust_disclaimer')}
      </p>
    </div>
  );
