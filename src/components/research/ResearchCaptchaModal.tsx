...
export function ResearchCaptchaModal({open,jobId,site,onComplete,onSkip,onClose}:Props){const{t}=useLanguage();const[shot,setShot]=useState<Shot|null>(null),[busy,setBusy]=useState(false),[err,setErr]=useState<string|null>(null),[finishing,setFinishing]=useState(false),[skipping,setSkipping]=useState(false);const img=useRef<HTMLImageElement|null>(null);
 ...
 return <div className="fixed inset-0 z-[120] bg-black/70 backdrop-blur-sm flex items-center justify-center p-0 sm:p-4 overscroll-contain">
   <div role="dialog" aria-modal="true" aria-label={t('verify_captcha_title')} className="w-full h-[100dvh] max-h-[100dvh] sm:h-auto sm:w-[min(1100px,94vw)] sm:max-h-[90vh] rounded-none sm:rounded-2xl border-0 sm:border border-border bg-background shadow-2xl overflow-hidden flex flex-col">
     <div className="px-3 py-3 sm:px-5 sm:py-4 border-b flex items-center gap-2 sm:gap-3 bg-card shrink-0">
       <ShieldCheck className="h-5 w-5 text-primary shrink-0"/>
       <div className="flex-1 min-w-0">
         <p className="text-sm font-semibold truncate">{t('verify_captcha_title')}</p>
         <p className="text-xs text-muted-foreground line-clamp-2 sm:line-clamp-none break-words">{site?`${site}: `:''}{t('verify_captcha_description')}</p>
       </div>
       <Button size="icon" variant="ghost" className="shrink-0" onClick={refresh} disabled={busy} aria-label={t('verify_captcha_loading')}><RefreshCw className={`h-4 w-4 ${busy?'animate-spin':''}`}/></Button>
       {onClose&&<Button size="icon" variant="ghost" className="shrink-0" onClick={onClose} aria-label={t('verify_captcha_skip_button')}><X className="h-4 w-4"/></Button>}
     </div>
     <div className="px-3 sm:px-5 py-2 border-b shrink-0 bg-muted/40 space-y-1.5">
       <p className="text-xs text-muted-foreground break-words">{t('verify_captcha_recommended_note')}</p>
       {shot?.humanAssist&&<p className="text-xs font-medium text-foreground break-words">{t('verify_captcha_extension_note')}</p>}
     </div>
