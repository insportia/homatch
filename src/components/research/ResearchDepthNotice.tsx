...
export function ResearchDepthNotice(){
 const {t}=useLanguage();
 return <section aria-label={t('verify_depth_title')} className="w-full rounded-2xl border border-border bg-card/60 p-4 sm:p-5"><div className="flex gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary/10"><Clock3 className="h-4 w-4 text-primary"/></div><div className="min-w-0"><div className="flex items-start gap-2"><p className="font-display text-lg font-bold tracking-[-0.012em] break-words">{t('verify_depth_title')}</p><ShieldCheck className="h-4 w-4 text-primary shrink-0 mt-0.5"/></div><p className="measure mt-1.5 text-base leading-relaxed text-ink-soft break-words">{t('verify_depth_body')}</p></div></div></section>;
}
