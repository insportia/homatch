  ...
  return (
    <div className="space-y-2 border-b py-4 last:border-b-0">
      <div className="flex items-center justify-between gap-2">
        <Label htmlFor={`f-${field.key}`} className="text-[14px] font-medium">
          {t(field.labelKey)}
        </Label>
        ...
        <div className="flex items-center gap-2">
          ...
          {LOCALES.filter(l => l !== locale).map(other => (
            <button
              ...
              className="flex items-center gap-1 rounded px-1 py-0.5 text-[12px] uppercase text-muted-foreground hover:bg-accent"
      ...
      {state === 'needs_update' && !suggestion && (
        <div className="flex items-center justify-between gap-2 rounded-md bg-amber-500/10 px-3 py-2">
          <span className="text-[13px] text-muted-foreground">{t('studio_state_needs_update')}</span>
          <Button
            variant="ghost" size="sm" className="h-6 px-2 text-[13px]"
      ...
      {suggestion && (
        <div className="space-y-2 rounded-md border border-sky-500/40 bg-sky-500/[0.06] p-3">
          <p className="text-[13px] font-medium text-muted-foreground">{t('studio_suggestion')}</p>
          <p className="whitespace-pre-wrap text-[15px] leading-relaxed">{suggestion}</p>
          <div className="flex gap-2">
            <Button
              size="sm" className="h-7 gap-1.5 px-2.5 text-[13px]"
            ...
            <Button
              variant="ghost" size="sm" className="h-7 gap-1.5 px-2.5 text-[13px]"
...
function MediaEditor({ studio, slot, labelKey }: { studio: StudioState; slot: string; labelKey: string }) {
  ...
  return (
    <div className="space-y-2 border-b py-4">
      <Label className="text-[14px] font-medium">{t(labelKey)}</Label>
      ...
      <div className="flex gap-2">
        ...
        {current && (
          <Button
            variant="ghost" size="sm" className="h-8 text-[13px]"
      ...
      {current && (
        <div className="space-y-1">
          <Label htmlFor={`alt-${slot}`} className="text-[13px] text-muted-foreground">
            {t('studio_asset_alt')}
          </Label>
          <Input
            id={`alt-${slot}`}
...
export function Inspector({ studio }: { studio: StudioState }) {
  ...
  if (!selected) {
    return (
      <div className="grid h-full place-items-center p-6 text-center text-[15px] text-muted-foreground">
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4">
        <div className="flex items-center justify-between border-b py-4">
          <Label htmlFor="sec-enabled" className="text-[14px] font-medium">
            {t('studio_enabled')}
          </Label>
          <Switch
            id="sec-enabled"
        ...
        {def.variants.length > 1 && (
          <div className="space-y-1.5 border-b py-4">
            <Label className="text-[14px] font-medium">{t('studio_variant')}</Label>
            <Select value={selected.variant} onValueChange={setVariant}>
              <SelectTrigger className="h-8 text-[14px]"><SelectValue /></SelectTrigger>
              <SelectContent>
                {def.variants.map(v => (
        ...
        {def.themes.length > 0 && (
          <div className="space-y-1.5 border-b py-4">
            <Label className="text-[14px] font-medium">{t('studio_theme')}</Label>
            <Select
        ...
        <div className="space-y-1.5 border-b py-4">
          <Label className="text-[14px] font-medium">{t('studio_spacing')}</Label>
          <Select
          ...
          >
            <SelectTrigger className="h-8 text-[14px]"><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="compact" className="text-[14px]">{t('studio_spacing_compact')}</SelectItem>
              <SelectItem value="normal" className="text-[14px]">{t('studio_spacing_normal')}</SelectItem>
              <SelectItem value="spacious" className="text-[14px]">{t('studio_spacing_spacious')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
