  ...
  return (
    <div className="flex flex-wrap items-center gap-2 border-b bg-card px-3 py-2">
      <Select value={slug} onValueChange={v => setSlug(v as PageSlug)}>
        ...
        <SelectContent>
          <SelectItem value="home" className="text-[12px]">{t('mp_nav_start')}</SelectItem>
          <SelectItem value="about" className="text-[12px]">{t('nav_about')}</SelectItem>
        </SelectContent>
      </Select>
      ...
      <Select value={mode} onValueChange={v => setMode(v as TranslationMode)}>
        ...
        <SelectContent>
          <SelectItem value="manual" className="text-[12px]">{t('studio_mode_manual')}</SelectItem>
          <SelectItem value="suggest" className="text-[12px]">{t('studio_mode_suggest')}</SelectItem>
          <SelectItem value="auto_all" className="text-[12px]">{t('studio_mode_auto')}</SelectItem>
        </SelectContent>
      </Select>
      ...
      <div className="ms-auto flex items-center gap-2">
        {dirty && (
          <span className="text-[11px] text-amber-600">{t('studio_unsaved')}</span>
        )}
        <Button
          variant="outline" size="sm" className="h-8 gap-1.5 text-[11px]"
