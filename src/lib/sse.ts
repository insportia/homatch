...
/*
 * A SIGNED-OUT CALLER STILL NEEDS AN AUTHORIZATION HEADER.
 *
 * With no accessToken this used to send `apikey` alone, and Supabase's
 * edge gateway rejects that with 401 before the function is even
 * reached when verify_jwt is on. homatch-ai was written expecting the
 * anon key as its bearer — its own comment says "the anon key is a
 * valid bearer here, so no user means an anonymous caller", and the
 * whole anonymous-session branch underneath it was unreachable from a
 * browser because the header never arrived. The anon key IS a JWT;
 * sending it is what supabase-js itself does.
 */
export async function sendStreamRequest(options:StreamRequestOptions):Promise<void>{const{functionUrl,requestBody,supabaseAnonKey,accessToken,onData,onMeta,onComplete,onError,signal}=options;const sseHook=createSSEHook({onData,onMeta,onCompleted:(err)=>(err?onError(err):onComplete()),onAborted:()=>console.log('Stream aborted')});try{await ky.post(functionUrl,{json:requestBody,headers:{Authorization:`Bearer ${accessToken??supabaseAnonKey}`,apikey:supabaseAnonKey,'Content-Type':'application/json'},timeout:90_000,signal,hooks:{afterResponse:[sseHook]}});}catch(err){if(!signal?.aborted)onError(err as Error);}}
