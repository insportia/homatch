...
function NotificationsContent() {
  ...
  const handleNotifClick = (notif: Notification) => { const meta = (notif.metadata ?? {}) as Record<string, unknown>; const kind = typeof meta.kind === 'string' ? meta.kind : ''; if (kind === 'NEW_MESSAGE') { const id = typeof meta.conversation_id === 'string' ? meta.conversation_id : ''; navigate(id ? `/chat?conversation=${encodeURIComponent(id)}` : '/chat'); return; } if (kind === 'VIEWING_REQUEST' || kind === 'VIEWING_UPDATE') { const id = typeof meta.viewing_request_id === 'string' ? meta.viewing_request_id : ''; navigate(id ? `/viewings?request=${encodeURIComponent(id)}` : '/viewings'); return; } if (notif.type === 'MATCH_AVAILABLE' || notif.type === 'MATCH_FOUND') { const propId = notif.property_id ?? (typeof meta.property_id === 'string' ? meta.property_id : undefined); if (propId) navigate(`/property/${propId}/matches`); return; } if (notif.type === 'CREDITS_TOPPED_UP' || notif.type === 'LOW_CREDITS' || notif.type === 'RESEARCH_PRODUCT_PURCHASED') { navigate('/credits'); return; } if (notif.property_id) navigate(`/property/${notif.property_id}`); };
