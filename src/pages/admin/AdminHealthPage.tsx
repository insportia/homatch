  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between text-xs">
        ...
        {blocked && <Badge variant="destructive" className="text-[13px] px-1.5 py-0 ml-1">BLOCKED</Badge>}
        {!blocked && pct >= 80 && <Badge variant="outline" className="text-[13px] px-1.5 py-0 ml-1 border-amber-500 text-amber-500">WARNING</Badge>}
      </div>
            ...
            {loading ? <Skeleton className="h-24" /> : health ? (
              <>
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <Badge variant={health.mock_mode_active ? 'destructive' : 'outline'} className="text-[13px] px-1.5">
              ...
              {Object.entries(health.provider_statuses).map(([name, p]) => (
                <div key={name} className="flex items-center justify-between gap-3 py-2 border-b border-border/30 last:border-0">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <Badge className={cn('text-[13px] px-1.5 py-0', PROVIDER_COLORS[p.status] ?? '')}>
