                ...
                {loading ? Array.from({ length: 8 }).map((_, i) => (
                  ...
                  <tr key={p.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                    ...
                    <td className="px-4 py-2.5 whitespace-nowrap">
                      <Badge variant="outline" className="text-[13px]">{p.provider ?? '—'}</Badge>
                    </td>
                    <td className="px-4 py-2.5 whitespace-nowrap">
                      <Badge variant={STATUS_VARIANT[p.status] ?? 'outline'} className="text-[13px]">{p.status ?? '—'}</Badge>
                    </td>
                    <td className="px-4 py-2.5 whitespace-nowrap">
                      <span className="font-mono text-[13px] text-muted-foreground truncate max-w-[140px] block">{p.idempotency_key ?? '—'}</span>
                    </td>
