            ...
            return (
              <Card key={h.provider} className={cn('shadow-sm', isDisabled && 'opacity-60 border-dashed')}>
                ...
                <CardContent className="pb-4 space-y-2">
                  ...
                  {h.last_error && (
                    <p className="text-[13px] text-destructive bg-destructive/10 rounded px-2 py-1 truncate" title={h.last_error}>
                      {h.last_error}
                    </p>
                  )}
                  {h.last_tested_at && (
            ...
            <Card key={p.provider_code} className={cn('shadow-sm', !p.enabled && 'opacity-70 border-dashed')}>
              <CardContent className="p-4 space-y-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge variant="outline" className={cn('text-[12px] px-1.5', p.health_status === 'ACTIVE' ? 'border-green-500/40 text-green-500' : p.health_status === 'LOCKED' ? 'border-destructive/40 text-destructive' : '')}>
                ...
                {p.notes && <p className="text-[13px] text-muted-foreground/80 leading-snug">{p.notes}</p>}
