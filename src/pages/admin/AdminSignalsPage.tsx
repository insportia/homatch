        ...
        <div className="space-y-4 text-xs">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <p className="font-mono text-[14px] break-all">{signal.id}</p>
            </div>
            <div>
              <p className="text-muted-foreground font-medium uppercase tracking-wide mb-0.5">{t('admin_signals_status')}</p>
              <Badge variant={STATUS_VARIANT[signal.classification_status] ?? 'outline'} className="text-[13px]">
          ...
          {signal.intent_json && (
            <div>
              ...
              <pre className="rounded-lg bg-secondary/50 border border-border p-3 max-h-64 overflow-auto text-[14px] font-mono leading-relaxed">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {failedCount > 0 && (
              <Badge variant="outline" className="text-[13px] px-1 py-0 border-amber-500/40 text-amber-500 ml-0.5">
                {failedCount}
              </Badge>
            )}
          </Button>
                ...
                {loading ? Array.from({ length: 10 }).map((_, i) => (
                  ...
                  <tr key={s.id} className="border-b border-border last:border-0 hover:bg-muted/30 cursor-pointer"
                    ...
                    <td className="px-4 py-2.5 max-w-[280px]">
                      ...
                      {s.error_message && (
                        <p className="text-[13px] text-destructive truncate mt-0.5">{s.error_message.slice(0, 60)}</p>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-2.5 whitespace-nowrap">
                      <Badge variant={STATUS_VARIANT[s.classification_status] ?? 'outline'} className="text-[13px]">
