...
serve(async(req)=>{
 ...
 try{
  ...
  let inserted=0,skipped=0;for(const x of results){if(!x.title&&!x.snippet){skipped++;continue}const text=`${x.title}\n${x.snippet}`.trim(),fp=await fingerprint(`${propertyId||'global'}:${text}`);const {error}=await db.from('raw_signals').insert({property_id:propertyId||null,source_id:sourceId,platform:'GOOGLE',external_id:x.url||fp,source_url:x.url||null,original_text:text,language,published_at:x.publishedAt,content_fingerprint:fp,provider:'DATAFORSEO',classification_status:'PENDING',mock_mode:false});if(error)skipped++;else inserted++}
...
async function fingerprint(t:string){const b=new TextEncoder().encode(t.toLowerCase().replace(/\s+/g,' ').trim().slice(0,400));const h=await crypto.subtle.digest('SHA-256',b);return Array.from(new Uint8Array(h)).slice(0,16).map(x=>x.toString(16).padStart(2,'0')).join('')}
