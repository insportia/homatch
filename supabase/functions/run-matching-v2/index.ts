...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  for(const ip of profiles||[]){
    ...
    const text=String(ip.original_text||ip.translated_text||'');
  ...
  if(best>0) await db.from('properties').update({matchability_score:best}).eq('id',p.id);
  return json({success:true,matchesCreated:created,matchesSkipped:skipped,bestScore:best,buckets});
 }catch(e){return json({error:e instanceof Error?e.message:String(e)},500)}
});

function score(p:any,f:any,ip:any){
 let s=0; const reasons:string[]=[]; const mismatches:string[]=[];
 const propTxn=norm(p.transaction_type), intTxn=norm(ip.transaction_type);
 const txnKnown=!!propTxn&&!!intTxn, txnMatch=!txnKnown||propTxn===intTxn;
 ...
 s+=Math.round(Math.min(1,Number(ip.intent_confidence||0))*5);
 let final=Math.max(0,Math.min(100,Math.round(s)));
 if(txnKnown&&!txnMatch) final=Math.min(final,49); if(typeKnown&&!typeMatch) final=Math.min(final,49);
