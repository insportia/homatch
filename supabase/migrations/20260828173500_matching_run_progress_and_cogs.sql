...
create index if not exists matching_run_progress_property_idx
  on public.matching_run_progress(property_id, started_at desc);
...
create policy matching_run_progress_owner_select
  on public.matching_run_progress
...
do $$ begin
  if not exists (
    ...
    where table_schema='public' and table_name='matches' and column_name='estimated_cogs_usd'
  ) then
    alter table public.matches add column estimated_cogs_usd numeric(12,6) not null default 0;
  end if;
  if not exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='matches' and column_name='pricing_multiplier'
  ) then
